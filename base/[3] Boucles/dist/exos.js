...
console.log("--- Exo 2 ---");
function countOccurrences(tableau, cible) {
    let compteur = 0;
...
function secondLargest(tableau) {
    if (tableau.length < 2)
        return null;
    let premier = -Infinity, second = -Infinity;
    for (let element of tableau) {
        if (element > premier) {
            second = premier;
            premier = element;
...
let results = countCharacters("Hello");
...
// caractères répétés.
console.log("--- Exo 10 ---");
function generatePermutations(chaine) {
    if (chaine.length <= 1)
        return [chaine];
    let chainePermut = new Set();
    for (let i = 0; i < chaine.length; i++) {
        let char = chaine[i];
        let charVariant = chaine.slice(0, i) + chaine.slice(i + 1);
        // Récursivité
